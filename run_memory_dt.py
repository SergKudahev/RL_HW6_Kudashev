...
def parse_args():
    ...
    parser.add_argument('--memory_type', type=str, default='gru',
                        choices=['gru', 'lstm', 'mlp_vector', 'none'],
                        help='Type of memory to use (gru, lstm, mlp_vector or none)')
...
def main():
    ...
    # Сохраняем train_losses и val_returns по эпохам
    df_train = pd.DataFrame({
        "epoch": list(range(1, len(train_losses)+1)),
        "train_loss": train_losses,
        "val_return": val_returns + [None]*(len(train_losses)-len(val_returns))
    })
    trainlog_path = f"results/{args.env}_dt_{args.memory_type}_trainlog.csv"
    df_train.to_csv(trainlog_path, index=False)
    print(f"Saved training log to {trainlog_path}")

    # Сохраняем returns по эпизодам
    df_eval = pd.DataFrame({
        "episode": list(range(1, len(returns)+1)),
        "reward": returns
    })
    evallog_path = f"results/{args.env}_dt_{args.memory_type}_eval_rewards.csv"
    df_eval.to_csv(evallog_path, index=False)
    print(f"Saved evaluation rewards to {evallog_path}")

    # Сохраняем mean_return и success_rate в отдельный файл
    summary_path = f"results/{args.env}_dt_{args.memory_type}_summary.csv"
