...
def parse_args():
    ...
    parser.add_argument('--memory_type', type=str, default='gru', choices=['gru', 'lstm', 'mlp_vector', 'none'],
...
def main():
    ...
    df = pd.DataFrame({"episode": list(range(1, len(total_rewards)+1)), "reward": total_rewards})
    result_path = f"results/{args.env}_dt_{args.memory_type}_rewards.csv"
    df.to_csv(result_path, index=False)
    print(f"Saved episode rewards to {result_path}")
